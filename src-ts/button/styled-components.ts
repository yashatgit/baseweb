...
export const BaseButton = styled(
  'button',
  ({$theme, $size, $kind, $shape, $isLoading, $isSelected, $disabled}:SharedStylePropsT) => ({
    display: 'inline-flex',
...
export const EndEnhancer = styled('div', ({$theme}:SharedStylePropsT) => ({
  display: 'flex',
  [$theme.direction === 'rtl' ? 'marginRight' : 'marginLeft']: $theme.sizing
    .scale500,
}));

export const StartEnhancer = styled('div', ({$theme}:SharedStylePropsT) => ({
  display: 'flex',
...
export const LoadingSpinner = styled(
  'div',
  ({$theme, $kind, $disabled}:SharedStylePropsT) => {
...
function getLoadingSpinnerColors({$theme, $kind, $disabled}:SharedStylePropsT) {
  if ($disabled) {
    return {
...
function getFontStyles({$theme, $size}:SharedStylePropsT) {
  switch ($size) {
    case SIZE.mini:
...
function getPaddingStyles({$theme, $size, $shape}:SharedStylePropsT) {
